...
- `#[cfg(version(cfg_name, since = "<version>"))]`, a minimum-version `cfg` predicate

Say this was added before 1.70, you could do:
```toml
[target.'cfg(not(version(rust, since = "1.70")))'.dependencies"]
is-terminal = "0.4.16"
```

```rust
fn is_stderr_terminal() -> bool {
    #[cfg(version(rust, since = "1.70"))]
    use std::io::IsTerminal as _;
    #[cfg(not(version(rust, since = "1.70")))]
...
- `--cfg rust` and `#[cfg(version)]` were stabilized in 1.20
...
```rust
#[cfg_attr(version(rust, since = "1.27"), must_use)]
fn double(x: i32) -> i32 {
    2 * x
}
...
> ```rust
> #[cfg_alias(must_use_exists, version(rust, since = "1.27"))]
>
...
the above code would error when compiling with your MSRV because the `version` predicate does not exist with that version.
However, the presence of `--cfg rust` implies that we are on 1.27,
so you can "detect" support for `version` by changing your code to:
```rust
#[cfg_attr(rust, cfg_attr(version(rust, since = "1.27"), must_use))]
fn double(x: i32) -> i32 {
    2 * x
}
...
ConfigurationVersion ->
  `version` `(` IDENTIFIER `,` `since` `=` ( STRING_LITERAL | RAW_STRING_LITERAL ) `)`
...
3. If any of the following evaluates to `true` for any cfg entry for `IDENTIFIER`, `version` will evaluate to `true`, otherwise `false`.
    ...
    3. Otherwise, if `IDENTIFIER`s value has the same or higher precedence, this entry will evaluate to `true`
       For example, `#[cfg(version(rust, since = "1.90"))]` will be interpreted as `precedence_of(1.95.2) >= precedence_of(1.90.0)`.
...
Examples:
- `cfg(version(unset_name, since = "1.0"))` will be false
- `--cfg name_only` and `cfg(version(name_only, since = "1.0"))` will be false
- `--cfg foo="bird"` and `cfg(version(name_only, since = "1.0"))` will be a compiler error
- `--cfg foo="1.1"` and `cfg(version(foo, "bird"))` will be a compiler error
- `--cfg foo="1.1"` and `cfg(version(foo, since = "1.0"))` will be true
- `--cfg foo="1.1"` and `cfg(version(foo, since = "1.2"))` will be false
- `--cfg foo --cfg foo="1.1" --cfg foo="1.0"` and `cfg(version(foo, since = "1.1"))` will be true
...
`rust` will be specified as `--check-cfg 'cfg(rust, values(version(since = "1.95")))'`
...
- Suggest `#[cfg(version)]` in the `clippy::incompatible_msrv` diagnostic report (maybe along with offering to bump MSRV as that is a reasonable alternative)
...
Ignoring the logic, a straight-English reading of `#[cfg(not(version(rust, since = "1.95")))]` is unnatural and could cause confusion.
...
this does not include a solution for adopting this within `Cargo.toml` without waiting for an MSRV bump to the version `version` is stabilized in.
...
Pre-releases of major versions isn't a consideration for `rust` but in the general use of `version`.

If wanting to split a continuous range with minor and patch versions,
`#[cfg(version(foo, since = "1.1"))]` and `#[cfg(not(version(foo, since = "1.1")))]`
works reasonably well.

The problem comes into play when doing so with major versions when pre-releases are involved,
like `#[cfg(version(foo, since = "2.0"))]` and `#[cfg(not(version(foo, since = "2.0")))]`.
In this situation, a `2.0.0-dev.5` will match the second condition when the user likely only wanted to include `1.*`.
Instead, they should do `#[cfg(version(foo, since = "2.0.0-0"))]` and `#[cfg(not(version(foo, since = "2.0.0-0")))]` or have a third case for pre-releases of `foo@2.0.0`.
...
## `version(since)` cfg predicate rationale

The `version(since)` name was taken from
[`rustversion::since`](https://crates.io/crates/rustversion) and the `#[deprecated(since)]` / `#[stable(since)]` attributes.
We need to express both what comparison is happening (`>=`) and the comparison algorithm (version sort).
This also leaves room for related predicates like `version(before)`.
...
We could also call this `version(minimum)` or `version(ge)`, or support comparison operators in the spirit of [RFC 3796](https://github.com/rust-lang/rfcs/pull/3796).
...
Having a more specific name like `version` can avoid these concerns.
We chose `version(since)` since it can read as "for versions of rust since 1.20", helps with version-related auto-completions, and version-related predicates will naturally nest inside the attribute.
However, the name `version(since)` can seem excessively verbose, especially compared to `version` or `since`.

We could swap the order of parameters and make `rust` a default for the second parameter to allow `#[cfg(version(since = "1.95"))]` as a shorthand. We can even make `since` the default and allow `#[cfg(version("1.95"))]`.
However, this would look confusing in Cargo and doesn't seem like it's offering enough of a benefit to be worth the costs.
...
For example, if `--cfg=foo="1.0"` caused `cfg(version(foo, since = "1.0"))` to be `false` and we later allowed `"1.0"` for the `IDENTIFIER`, it would now be `true` and would change behavior.
Because we'll have `version(rust, since = _)` at that point, it won't require an MSRV bump.
...
Another benefit to erroring is so `not(version(invalid, since = "<invalid>"))` is not `true`.
...
However, this would also apply to a `version(before)` predicate, making `version(before)` not the same as `not(version(since))`.
...
This can have side effects like allowing comparing words like with `#[cfg(version(hello, since = "world"))]`,
...
(e.g. `#[cfg(version(rust, since = 1.95.0))]`).
If we ever decided to support operators (e.g.`#[cfg(version(rust, "=1.95"))]`, see `--check-cfg`), then we'd need to decide if those also go outside the string or then require a string, being inconsistent.
...
A user could do `--cfg=foo --cfg=foo="1.2" --cfg=foo"1.3"`, leading `foo` to be a set of:
- `("foo", None)`
- `("foo", "1.2")`
- `("foo", "1.3")`
...
We take this into account by checking if any cfg with the name `foo` matches the version.
Alternatively, we could fail the match in this case but that prevents `--cfg rust` from checking if this feature is stable.
...
However,
- the `version` predicates are a general feature intended to be used with other version numbers where exact matches may also be appropriate.
...
allow operators inside of the cfg `version` predicate, e.g. `#[cfg(version(rust, equals = "1.95"))]`.
...
Alternatively, we could add a separate cfg, like `has_rust`, `rust_is_set`, `has_version`.
...
The compiler may choose to offer an unstable flag to mark a nightly as "complete" to allow for testing of features with `version`.
...
Instead of having a named parameter for the predicate,
...
This splits our specific predicate name (`version(since)`) into smaller parts without losing conveyed meaning.
...
- `version(rust, since = "1.95")`, `version(rust, ">=1.95")`, `version(rust >= "1.95")`, or `version(rust,
since("1.95"))`
...
Instead of requiring the `IDENTIFIER` in the cfg `version` predicate to be strictly `Version` `major.minor`,
...
The version requirement (string literal) for cfg `version` and check-cfg `version` would be similarly updated
except  `BuildMetadata` would not be allowed.
A user would see the `unexpected_cfgs` lint if their cfg `version` string literal had more precision (more `VersionField`s) than the check-cfg `version` predicate.
...
> ```rust
> #[cfg_attr(version(rust, since = "1.27.0-0"), must_use)]
> fn double(x: i32) -> i32 {
>     2 * x
> }
>
...
`--check-cfg cfg(edition, values("2015", "2018", "2021", "2024", version(since = "2025")))`
- The discrete values for known editions is there to help catch mistakes
- `version(since = "2025")` is used so packages don't have to deal with `unexpected_cfgs` when operating with edition versions higher than their current compiler recognizes and without having to try to predict what our future edition versions and policies may be
- `version(since = "2025")` also ensures that a user gets an `unexpected_cfgs` warning if they do `cfg(version(edition, since = "2028.10"))` as that matches the `version(since = "2025")` but has more precision
...
This would require the check-cfg `version` to use the appropriate amount of precision to not warn.

If this is still not sufficient, we some options include:
- Add an optional third field for specifying the version format (e.g. `#[cfg(version(windows, since = "10.0.10240", <policy-name>)]`)
...
We may want to allow `#[cfg(version(serde, since = "1.0.900")]`.
...
## Clippy lint: prefer using `cfg(version)` via `cfg_alias!`

Once we have this RFC and [RFC 3804](https://github.com/rust-lang/rfcs/pull/3804),
we may want a restriction lint that would encourage people to use `cfg(version)` through `cfg_alias`
